...
public class ImaGesApp {
    ...
    private static final String USAGE = "Usage: java -cp ccd-algorithm.jar "
            + "edu.pitt.dbmi.ccd.algorithm.tetrad.ImaGesApp "
            + "--data <file> "
            + "[--out <dir>] "
            + "[--delimiter <char>] "
    ...
    private static final String HELP_INFO = "================================================================================\n"
            ...
            + String.format("%-18s\t%s\n", PENALTY_DISCOUNT_PARAM, "Penality discount.  The default value is 1.0.")
            + String.format("%-18s\t%s\n", DEPTH_PARAM, "The search depth.  The default value is 2.")
	...
	public static void main(String[] args) {
        ...
        if (outputFileName == null) {
            outputFileName = String.format("images_%d.txt", System.currentTimeMillis());
        } else {
        ...
        try (PrintStream stream = new PrintStream(new BufferedOutputStream(Files.newOutputStream(outputFile, StandardOpenOption.CREATE)))) {
            ...
            algorithm.run(FastImages2.class, null, dataset, params);
            stream.flush();
