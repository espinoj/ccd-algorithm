...
/**
 *
 * Jun 4, 2015 9:24:02 AM
 *
 * @author Kevin V. Bui (kvb2@pitt.edu)
 */
public class FastGesApp {

    private static final String USAGE = "Usage: java -jar fastges-cli.jar "
            + "--data <file> "
            + "[--out <dir>] "
            + "[--delimiter <char>] "
    ...
    private static final String HELP_INFO = "================================================================================\n"
            ...
            + String.format("%-18s\t%s\n", GRAPHML_FLAG, "Output the graph as a file in a GraphML format.  Not created by default.")
    ...
    public static void main(String[] args) {
        ...
        if (outputFileName == null) {
            outputFileName = String.format("ges_%d", System.currentTimeMillis());
        }

        Algorithm algorithm = new TetradAlgorithm();
        Path txtOutputFile = Paths.get(dirOut.toString(), outputFileName + ".txt");
        try (PrintStream stream = new PrintStream(new BufferedOutputStream(Files.newOutputStream(txtOutputFile, StandardOpenOption.CREATE)))) {
        ...
        if (outputGraphML) {
            Path graphmlOutputFile = Paths.get(dirOut.toString(), outputFileName + ".graphml");
            try (PrintStream stream = new PrintStream(new BufferedOutputStream(Files.newOutputStream(graphmlOutputFile, StandardOpenOption.CREATE)))) {
                GraphIO.write(algorithm.getGraph(), GraphIO.GraphOutputType.GRAPHML, stream, outputFileName);
            } catch (Exception exception) {
                exception.printStackTrace(System.err);
            }
        }
